...
parser.add_argument("--environments", required=False, default="dev,tst,prd", help="Comma seperated list of environments to setup.")
...
parser.add_argument("--output_path", required=False, default="../parameters/parameter.yml", help="Path to where the parameter file should be saved.")
parser.add_argument('--mode', choices=['file', 'live'], required=True, help='Data source mode. Set to "file" if enviroments are fetched from a file. Set to "live" if enviroments are retrieved live from Fabric.')
parser.add_argument('--folder_path', help='Path to the environment files (required if mode is "file")')
...
if args.mode == 'file' and not folder_path:
    parser.error('--folder_path is required when --mode is "file"')

if args.mode == "file":
    env_file_map = {}
    for environment in environments:
        env_file_map[environment] = f'{folder_path}/env_data_{environment}.json'

    all_environments = miscfunc.load_environments_as_dict(env_file_map)
    ...
    for environment in environments:
        ...
        all_environments[environment] = data

# Generate parameter file
yaml_file = os.path.join(os.path.dirname(__file__), output_path)
yml_data = miscfunc.build_parameter_yml(yaml_file, all_environments)
